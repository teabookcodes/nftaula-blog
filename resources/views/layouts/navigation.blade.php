    ...
    <div class="container mx-auto px-4 xl:px-0 py-2 flex items-center justify-between">
        ...
        <ul class="inline-flex space-x-8">
          {{-- {menuItems.map((item) => (
            <li key={item.name}>
              <Link
                href={item.href}
                className="inline-flex items-center text-base hover:text-primary"
              >
                {item.name}
              </Link>
            </li>
      ...
      <div class="hidden lg:flex lg:w-1/3 items-center justify-end gap-4">
        ...
        {authStatus ? (
          <Link
            href={"/profile"}
            class="p-2 text-gray-800 dark:text-gray-200 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-xl transition"
          >
            <svg
