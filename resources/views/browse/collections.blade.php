        ...
        <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 sm:px-3 pb-6 relative">
            <div class="z-10 absolute top-0 right-0 sm:right-3 flex gap-2 p-4 rounded-xl">
                ...
                <x-menubar-link :active="false"
                    href="https://twitter.com/intent/tweet?url={{ config('app.url') }}/collection/{{ $collection->id }}&text=Check%20out%20the%20{{ urlencode($collection->name) }}%20on%20NFTaula.io&via=NFTaula&media={{ config('app.url') }}/storage/collection_images/{{ $collection->image }}"
