    ...
    <div class="w-full flex flex-wrap text-gray-600 dark:text-gray-400">
        ...
        <div class="w-full flex flex-wrap text-gray-600 dark:text-gray-400">
            ...
            <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 sm:px-3 pb-6 relative">
                ...
                <a href="{{ route('nft-detail', $savedNft->nft) }}">
            ...
            <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 sm:px-3 pb-6 relative">
                <div class="z-10 absolute top-0 right-0 sm:right-3 flex gap-2 p-4 rounded-xl">
                    <form method="POST"
                        action="{{ $savedCollection->collection->savedCollections->where('user_id', auth()->id())->count() > 0 ? route('collections.unsave', $savedCollection->collection) : route('collections.save', $savedCollection->collection) }}">
                        @csrf
                        @if($savedCollection->collection->savedCollections->where('user_id', auth()->id())->count() > 0)
                        @method('DELETE')
                        @endif

                        <button type="submit"
                            class="p-2 rounded-full {{ $savedCollection->collection->savedCollections->where('user_id', auth()->id())->count() > 0 ? 'bg-indigo-500 text-white' : 'bg-gray-100 dark:bg-gray-900' }}">
                            <x-feathericon-bookmark class="w-4 h-4" />
                        </button>
                    </form>
                ...
                <a href="{{ route('collection-detail', $savedCollection->collection) }}">
                    <div class="p-4 bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl">
                        <p class="text-lg font-medium text-gray-900 dark:text-gray-100">{{
                            $savedCollection->collection->name }}</p>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">{{
                            $savedCollection->collection->category }}</p>
                        <img src="{{ asset('storage/collection_images/' . $savedCollection->collection->image) }}"
                            class="w-full h-56 object-cover mt-4 rounded-lg" />
                        <div class="absolute bottom-10 left-2 sm:left-5 p-4 rounded-xl">
                            <span
                                class="text-sm font-medium text-gray-800 dark:text-gray-200 py-2 px-3 bg-gray-100/80 dark:bg-gray-900/80 rounded-full">{{
                                $savedCollection->collection->floor_price }} <span class="uppercase">{{
                                    $savedCollection->collection->currency
                                    }}</span></span>
                        </div>
                    </div>
                </a>
