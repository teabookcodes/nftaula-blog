<x-app-layout>
    ...
    <div class="flex flex-col gap-8">
        <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow rounded-xl">
            <form method="POST" enctype="multipart/form-data" action="{{ route('collections.store') }}">
                @csrf
                <div class="w-full flex flex-col md:flex-row gap-0 md:gap-8">
                    <div class="w-full md:w-1/2">
                        ...
                        {{-- Price --}}
                        <x-input-label for="floor_price" :value="__('Floor Price')" class="mt-4" />
                        <x-text-input id="floor_price" name="floor_price" type="number" step="any"
                            placeholder="Floor price" class="mt-2 block w-full" :value="old('floor_price')" />
                        <x-input-error class="mt-2" :messages="$errors->get('floor_price')" />
                    ...
                    <div class="w-full md:w-1/2">
                        ...
                        <x-text-input id="marketplace_link" name="marketplace_link" type="text"
                            placeholder="Link to your collection on a marketplace" class="mt-2 block w-full"
                            :value="old('marketplace_link')" />
                        <x-input-error class="mt-2" :messages="$errors->get('marketplace_link')" />

                        {{-- Twitter link --}}
                        <x-input-label for="twitter_link" :value="__('Twitter URL')" class="mt-4" />
                        <x-text-input id="twitter_link" name="twitter_link" type="text"
                            placeholder="Link to your collection on Twitter" class="mt-2 block w-full"
                            :value="old('twitter_link')" />
                        <x-input-error class="mt-2" :messages="$errors->get('twitter_link')" />

                        {{-- Website link --}}
                        <x-input-label for="website_link" :value="__('Website URL')" class="mt-4" />
                        <x-text-input id="website_link" name="website_link" type="text"
                            placeholder="Link to your collection's website" class="mt-2 block w-full"
