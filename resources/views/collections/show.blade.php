        ...
        <div class="container px-5 py-6 md:py-24 mx-auto">
            <div class="mx-auto flex flex-wrap">
                <img alt="{{ $collection->name }}"
                    ...
                    src="/storage/collection_images/{{ $collection->image }}">
                <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                    <h1 class="text-gray-900 dark:text-white text-3xl title-font font-medium mb-1">{{ $collection->name
                        }}</h1>
                    <div class="flex mb-4">
                    ...
                    <div class="flex border-t border-gray-200 dark:border-gray-800 py-2">
                        ...
                        <span class="ml-auto text-gray-900 dark:text-white">{{ $collection->category }}</span>
                    </div>
                    <div class="flex border-t border-gray-200 dark:border-gray-800 py-2">
                        <span class="text-gray-500">Blockchain</span>
                        <span class="ml-auto text-gray-900 dark:text-white">{{ $collection->blockchain
                            }}</span>
                    </div>
                    <div class="flex border-t border-b mb-6 border-gray-200 dark:border-gray-800 py-2">
                        ...
                        <span class="ml-auto text-gray-900 dark:text-white">{{
                            $collection->created_using_ai ? 'Yes' : 'No' }}</span>
                    </div>
                    <div class="flex flex-col md:flex-row justify-between md:items-center">
                        <span class="title-font font-medium text-2xl text-gray-900 dark:text-white">{{
                            $collection->floor_price . " "
                            . $collection->currency }}</span>
                        <div class="flex mt-4 md:mt-0">
                            <a href="{{ $collection->marketplace_link }}"
                                class="flex flex-grow md:ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Check
                                out
                                on {{ $collection->marketplace }}</a>
