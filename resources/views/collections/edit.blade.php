            ...
            <div class="max-w-xl">
                <form method="POST" action="{{ route('collections.update', $collection) }}">
                    ...
                    <x-text-input id="name" name="name" type="text" placeholder="Name" class="mt-2 block w-full"
                        :value="old('name', $collection->name)" required autofocus autocomplete="name" />
                    <x-input-error class="mt-2" :messages="$errors->get('name')" />

                    <x-input-label for="description" class="mt-4" :value="__('Description')" />
                    <textarea name="description"
                        placeholder="{{ __('If you want, you can describe your collection here') }}"
                        class="form-textarea mt-2 w-full border-none rounded-xl shadow-sm bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">{{ old('description', $collection->description) }}</textarea>
